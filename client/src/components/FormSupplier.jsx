...
const FormOrders = ({  currentId, setcurrentId }) => {
  ...
  const [PostNew, setPostNew] = useState({fullname: "",email:"",age:"",phone: "",address: "", gender: "",company:""});

      const clear = () => {
        if (currentId) {
          setcurrentId(0);
        }
        setPostNew({fullname: "",email:"",age:"",phone: "",address: "", gender: "",company:""});
        
      };
    const Handlsubmit = (e) => {
        e.preventDefault();
        if (currentId) {
          dispatch(updatesupplier(PostNew, currentId));
        } else {
          dispatch(creatsuppliers(PostNew));
        }
        clear();
      };
      useEffect(() => {
    ...
    return (
      ...
      label="company"
      variant="outlined"
      value={PostNew.company}
      onChange={(e) =>
        setPostNew({ ...PostNew, company: e.target.value })
      }
    />
